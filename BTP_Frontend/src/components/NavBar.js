...
import { ExitToApp as ExitToAppIcon, Person as PersonIcon, AccountBalance as AccountBalanceIcon } from "@material-ui/icons";
    ...
    <nav className="flex justify-start items-center h-20 box shadow-md  px-8 gap-8 bg-[#1B3058] w-full">
      ...
      {auth.token && (
          <div className="flex items-center">
            ...
            <div className="text-white font-bold text-lg ml-2">{auth.username}</div>
            
            { !auth.isAdmin && (
              <>
              <AccountBalanceIcon style={{ fontSize: 24, color: "white", marginLeft: "10px" }} />
              <div className="text-white font-bold text-lg ml-2">{auth.branch}</div>
              </>
            )
