...
const MatchColumns = () => {
    ...
    const sendSelectedColumnNames=()=>{
        ...
        setIsLoading(true);
        axios.post(`${serverLink}/api/initialise/saveToDataBase`,{result:columnNamesMatched},{
            withCredentials: false,
            headers: {
              Accept: "application/json",
              "Content-Type": "application/json",
    ...
    const handleDownload=()=>{
        axios
        .get(`${serverLink}/api/initialise/modifiedFile`, {
            responseType: 'blob',
          },{
          withCredentials: false,
          headers: {
        ...
        <div className='flex flex-col max-w-[800px] min-w-[80%] shadow-lg rounded-xl '>
            ...
            {!fileExists && columnNamesMatched!==null && !isLoading &&
                <div className='flex justify-center rounded-b-xl items-center gap-2 p-2 box-border h-[500px] flex-wrap overflow-y-scroll'>
                    {!fileExists && columnNamesMatched!=null && Object.keys(columnNamesMatched).map((columnName)=>{
                        return(
                            <div className='w-[250px] flex flex-col justify-center rounded-md p-2 shadow-md'>
                                ...
                                <SelectBox uploadedColumnName={columnName} predictedColumnName={columnNamesMatched[columnName]} changeState={changeColumnNamesMatchedState}/>
                            </div>
                        )
                    })}
                </div>
            ...
            <div className='flex items-center flex-col h-[180px] gap-1'> 
                <img src={documentImage} alt="Not Found" style={{width:"200px",height:"120px"}}/>
                <p className='text-xl text-grey '>Hmm looks like you have already Matched the Columns</p>
            </div>
            }   
            {!fileExists && columnNamesMatched===null && !isLoading &&
            ...
            <div className='w-full p-2 flex justify-center'>
                {!fileExists && columnNamesMatched!=null && <Button variant='contained' startIcon={<FileUploadIcon/>} style={{margin:"auto",marginBottom:"5px"}} 
                onClick={sendSelectedColumnNames}>
                Save To DataBase</Button>}
                {!fileExists && columnNamesMatched==null && <Button variant="contained" onClick={getMatchedColumnNames} style={{margin:"auto",marginBottom:"5px"}} >Get Column Names</Button>}
                {fileExists && <Button variant='contained' startIcon={<DownloadIcon/>} style={{margin:"auto",marginBottom:"5px"}} onClick={handleDownload}>Download The Edited File</Button>}
            </div>
        </div>
    );
};
