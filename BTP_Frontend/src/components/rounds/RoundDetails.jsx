...
function RoundDetails(props) {
    ...
    const generateoffers=async ()=>{
        try {
            setIsLoading(true);
            let res=await axios.get(`${serverLink}/api/rounds/generateOffers/${props.roundNumber}`,{
        ...
        <div className='w-full h-auto flex justify-center items-center'>
            ...
            <div className='w-full flex flex-col justify-center items-center h-auto p-3'>
                <div className="flex w-[95%] border-2 h-[60px] mt-5 items-center justify-between p-3 bg-zinc-50 shadow-md rounded-md">
                    {!offersGenerated && <p className='text-xl text-[#1B3058]'>Generate Offers</p>}
                    {offersGenerated && <p className='text-xl text-[#1B3058]'>Offers Have Been Generated</p>}
                    <div className='flex w-[70%] justify-end items-center gap-5'>
                        { !offersGenerated && <Button variant="contained" onClick={generateoffers}>Generate</Button>}
                        { offersGenerated && <Button variant="contained">Regenerate</Button>}
                        {offersGenerated && <Button variant="contained" endIcon={<Download/>} onClick={handleDownloadOffersFile}>Download</Button>}
                    </div>
                </div>
                <div className='h-10 border-2'>
                </div>
                <div className='flex flex-col w-full items-center justify-center'>
                    <h1 className='text-2xl flex w-[95%] justify-center  p-2 bg-[#1B3058] text-yellow-400'>Updates From Round</h1>
                    ...
                    <div className="flex flex-col justify-center items-center w-[95%]  h-auto box-border p-3 rounded-md gap-2 border-2">
                         <RoundUpdateFiles 
                            fileName={"IITGCandidateDecision"} isPresent={roundStatus.IITGCandidateDecision} 
                            displayFileName={"IIT Goa Offered Candidate Decision File"}
                            roundNumber={props.roundNumber}/>
                        <div className='h-6 border-2'></div>
                        <RoundUpdateFiles fileName={"IITGOfferedButNotInterested"} 
                        ...
                        <RoundUpdateFiles 
                            ...
                            fileName={"ConsolidatedFile"} isPresent={roundStatus.ConsolidatedFile} roundNumber={props.roundNumber}/>
                        {roundStatus!=null && !offersGenerated && <p>Offers Not Generated</p>}
                    </div>}  
                    {roundStatus!=null && !offersGenerated &&
