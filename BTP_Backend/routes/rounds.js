...
const generatedOffersDirectoryPath=path.join(__dirname,'..','files','generatedOffers');
const updatesFromRoundsDirectoryPath=path.join(__dirname,'..','files','roundUpdates');
const {updateStatusIITGList}=require("../utils/updateStatusIITGList.js");
const {updateStatusIITGNotInterested}=require("../utils/updateStatusIITGNotIntersted.js");
const {updateStatusConsolidatedFile}=require("../utils/updateStatusConsolidatedFile.js");
...
router.get("/getRounds", async(req, res) => {
    ...
    try {
        ...
        updatesFromRoundsFiles=await getFilesInDirectory(updatesFromRoundsDirectoryPath);
    } catch (error) {
        console.log(error);
        res.status(500).send({"result":"Internal Server Error"});
    }
    ...
    if(updatesFromRoundsFiles.includes('.gitignore')){
        updatesFromRoundsFiles.splice(updatesFromRoundsFiles.indexOf('.gitignore'), 1); 
    }
    // console.log(OffersfilesList,updatesFromRoundsFiles);
    if(OffersfilesList.length===0 || updatesFromRoundsFiles.length===(3*OffersfilesList.length)){
...
*/ 
router.get("/getRoundDetails/:roundId", async(req, res) => {
    ...
    //checking if a offers are generated or not
    if( OffersfilesList.includes(`round${roundId}.xlsx`)){
        console.log(`round${roundId}.xlsx`);
        response.offersGenerated=true;
    }
    //checking if any updates have happend or not
    if(updatesFromRoundsFiles.includes(`round${roundId}_IITGCandidateDecision.xlsx`)) {
        response.IITGCandidateDecision=true;
    }
    if(updatesFromRoundsFiles.includes(`round${roundId}_IITGOfferedButNotInterested.xlsx`)) {
        response.IITGOfferedButNotInterested=true;
    }
    if(updatesFromRoundsFiles.includes(`round${roundId}_ConsolidatedFile.xlsx`)) {
...
*/ 
router.get("/generateOffers/:roundId", async(req, res) => {
    let roundId=req.params.roundId;
    // console.log("generate",roundId);
    console.log(`generating... round ${roundId} results`);
    try {
        fs.writeFile(`${generatedOffersDirectoryPath}/round${roundId}.xlsx`,``,(error)=>{
            if(error){
                console.log(error);
            }
        })
        
        let generated=await generateOffers(process.env.MYSQL_DATABASE,roundId,`${generatedOffersDirectoryPath}/round${roundId}.xlsx`); 
        res.status(200).send({"result":"Offers Generated Succesfully"});
    } catch (error) {
        try {
            let res=await resetRound(roundId);
        } catch (error) {
            console.log(error);
        }
        res.status(500).send({"result":"Internal Server Error"});
        ...
        fs1.move(oldpath, newpath, async function (err) {
            ...
            try {
                if(fileName==='IITGCandidateDecision'){
                    updateStatusIITGList(process.env.MYSQL_DATABASE,newpath,roundNumber,coapIdColumnName,candidateDecisonColumnName);
                }
                else if(fileName==='IITGOfferedButNotInterested'){
                    updateStatusIITGNotInterested(process.env.MYSQL_DATABASE,newpath,roundNumber,coapIdColumnName,candidateDecisonColumnName);
                }
                else if(fileName==='ConsolidatedFile'){
                    updateStatusConsolidatedFile(process.env.MYSQL_DATABASE,newpath,roundNumber,coapIdColumnName,candidateDecisonColumnName);
                }
                res.status(200).send({});
            } catch (error) {
                res.status(500).send({});
...
*/ 
router.get("/getFile/:roundNumber", async (req, res) => {
    let roundNumber=req.params.roundNumber;
...
*/ 
router.get("/getFile/:fileName/:roundNumber", async (req, res) => {
    let roundNumber=req.params.roundNumber;
    let queriedfileName=req.params.fileName;
...
*/ 
router.get("/reset/:roundNumber", async (req, res) => {
    let roundNumber=req.params.roundNumber;
    try {
        let res=await resetRound(roundNumber);
...
/*
    ...
    outgoing data: saved generated file
*/ 
router.post("/getColumnNames", async (req, res) => {
    var form = new formidable.IncomingForm();
    //parsing file using formiddable library
    let x=form.parse(req, async function (err, fields, files) {
        var oldpath = files.file.filepath;
        ...
        fs1.move(oldpath, newpath, async function (err) {
            ...
            try {
                ...
                var applicantsData=XLSX.utils.sheet_to_json(statussheet,{ header: 1 });
