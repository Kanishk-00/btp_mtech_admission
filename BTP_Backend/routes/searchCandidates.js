...
router.get("/getCoapIds", async (req, res) => {
    var con;
    try {
    ...
    try {
        const [coapIdsList]=await con.query(`SELECT COAP as label from mtechappl `);
...
router.post("/getinfo", async (req, res) => {
    ...
    try {
        const [filteredList]=await con.query(`SELECT COAP as coap,gender as gender,Category as category,
        maxgatescore as maxgatescore,ews as ews,pwd as pwd,AppNo as appno
        from mtechappl where
       category REGEXP ${(category===null ||category==='') ?`'\w*'`:`'${category}'`} 
       and COAP REGEXP ${(coapId===null||coapId==='')?`'\w*'`:`'${coapId}'`} 
       and gender REGEXP ${(gender===null||gender==='')?`'\w*'`:`'${gender}'`} order by maxGateScore DESC limit 50; `);
...
router.get("/getinfo/:coapid", async (req, res) => {
    ...
    try {
        const [filteredList]=await con.query(`select FullName,AppNo as ApplicationNumber,mtechappl.COAP,Email,MaxGateScore,Gender, Category,
